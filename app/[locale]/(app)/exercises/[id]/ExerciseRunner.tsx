...
export default function ExerciseRunner({ exercise }: Props) {
  ...
  if (!plugin) return <p className="text-sm text-red-600">Unknown exercise type: {exercise.type}</p>;

  const Renderer = plugin.Renderer as React.ComponentType<{ prompt: unknown; onSubmit: (a: unknown) => void }>;

  const handleSubmit = async (answer: unknown) => {
    setLoading(true);
    try {
      // client-side evaluate for instant feedback
      const evalResult = plugin.evaluate(exercise.prompt as never, exercise.solution, answer as never);
  ...
  return (
    <div className="rounded-xl border bg-white p-6 dark:bg-gray-900">
      ...
      {loading ? <p className="mt-3 text-sm text-gray-500">Evaluating...</p> : null}
      {result ? (
        <div className={`mt-4 rounded p-3 text-sm ${result.score >= 70 ? "bg-green-50 text-green-800" : "bg-amber-50 text-amber-800"}`}>
