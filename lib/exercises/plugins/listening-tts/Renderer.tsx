...
export function ListeningTtsRenderer({ prompt, onSubmit }: { prompt: ListeningTtsPrompt; onSubmit: (a: ListeningTtsAnswer) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">Listen and answer:</p>
      <p className="font-medium">{prompt.question}</p>
      {prompt.options ? (
        <div className="space-y-2">{prompt.options.map((o) => <label key={o} className="flex items-center gap-2 rounded border p-2"><input type="radio" name="ans" value={o} checked={answer === o} onChange={() => setAnswer(o)} />{o}</label>)}</div>
      ) : (
